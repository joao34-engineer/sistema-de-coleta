...
Happy path do celular: finalize / cancel / reopen (Fase 1A) / revise no mesmo processo Next — `after()` chama o DAL `processQueuedDocumentRenders` (lote 2 pdf+qr **daquele documento**) e **devolve essa Promise** para o isolate serverless não congelar no meio do claim. Sem secret no aparelho e sem `fetch` da própria `/api`. Esta rota HTTP e o cron são **retry/recovery** FIFO global, não o fluxo do dia a dia.
