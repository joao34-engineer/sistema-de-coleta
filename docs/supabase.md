...
O projeto usara, nesta etapa, um unico projeto Supabase remoto proprio para desenvolvimento integrado, validacao controlada e publicacao do MVP. Docker esta fora deste projeto: o fluxo operacional usa so o remoto linkado; nao ha `supabase start` nem `supabase test db`. Nao havera staging remoto nesta etapa.
