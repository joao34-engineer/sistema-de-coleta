...
A entrada na oficina exige conferencia item a item, quantidade observada, condicao observada e registro de divergencia quando necessario. Item que **não chegou** entra no payload com `arrival_status = missing` (qtd 0, `condition_observed = nao_recebido`, motivo obrigatório); permanece na guia e **não** entra em orçamento, progresso nem entrega. Se todos os itens vivos forem missing, a coleta vai a `delivered` no mesmo check-in. O evento deve guardar a assinatura da administradora responsavel com nome e CNPJ, mais `arrivedCount` / `missingCount` / `missingItemIds`.

A entrega ao cliente cria um evento e uma versao documental com somente os itens que estiverem prontos e forem efetivamente entregues. O cliente confere os itens e assina com nome e CNPJ. Entregas parciais sao permitidas; a coleta so assume o estado final `entregue` quando nao restarem itens **entregaveis** (vivos, ainda nao no termo, **com** linha de ordem de servico). Item sem linha de OS nao bloqueia `delivered`. A RPC de entrega aceita `em_reparo` (`in_service`), `pronta` (`ready`), `faturada` (`invoiced`) ou `entrega_parcial` (scan **5.6**, 06/09/2026, remoto + Production). Entrega mid-repair e permitida: so entra no termo o item cuja ordem de servico esta `pronto` em **todas** as linhas OS daquele item; item que nao esta `pronto` e recusado com `item_not_ready`. Unique parcial `service_order_items_collection_item_uidx` impede duas linhas OS para o mesmo item; `create_technical_budget` recusa `item_id` duplicado no payload (`duplicate_budget_item`). Apos entrega parcial, o status da OS segue o restante (`delivered` se nao sobrou nada entregavel, `in_service` se o restante inclui `em_reparo`, `ready` se o restante esta todo `pronto`). `update_service_progress` aceita coleta em `partial_delivery` ou `invoiced` e nao a puxa de volta para `ready`; recusa item ja presente em `delivery_items` com `item_already_delivered`. Registrar NF-e e opcional e nao e pre-requisito. **Decisao 07/09/2026 (fechada):** NF-e continua disponivel em `partial_delivery`; `register_invoice_reference` aceita `ready | partial_delivery`; coleta ja `invoiced` nao rebaixa para `partial_delivery` numa entrega posterior. A aprovacao do orcamento exige nome e CNPJ; o pad de assinatura desenhada fica **opcional** ate pedido explicito (scan **5.7**, 06/09/2026).
...
- A administradora pode reabrir uma coleta cancelada quando o cancelamento tiver sido indevido ou precisar ser desfeito. A reabertura registra ator, data, motivo e estado anterior; o mesmo codigo oficial permanece vinculado a coleta e nenhum numero e reutilizado. Se ja existir guia emitida, a reabertura gera nova versao documental e preserva a versao que registrava o cancelamento. Os eventos `collection.canceled` / `collection.reopened` gravam na metadata o status anterior da OS (`serviceOrderPreviousStatus`) e, no reopen, o status restaurado (`serviceOrderStatus`); a coluna `service_orders.previous_status_before_cancellation` continua a fonte da verdade do reopen.
