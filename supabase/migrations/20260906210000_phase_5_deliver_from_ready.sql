-- Fase 5: 5.6 entrega a partir de Pronto; NF-e opcional.
-- Aditiva: CREATE OR REPLACE na mesma assinatura de 9 argumentos (preserva grants Chat 3).
-- Sem DROP FUNCTION. Sem unique em delivery_items.
-- Corpo parte de 20260906170000 (5.5). Unica mudanca: ready entra no conjunto
-- que pode entregar. invoiced e partial_delivery continuam validos.
-- Nao altera register_invoice_reference: NF-e segue disponivel em ready.
...
begin
  ...
  if collection_record.status not in ('ready', 'invoiced', 'partial_delivery') then
