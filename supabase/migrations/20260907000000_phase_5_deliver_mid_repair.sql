-- Fase 5: entrega parcial mid-repair (itens Pronto com outros ainda em reparo).
-- Aditiva: CREATE OR REPLACE nas mesmas assinaturas (preserva grants Chat 3).
-- Sem DROP FUNCTION. Sem GRANT. Sem alteracao de CHECK nem de dados.
-- Corpos partem de 20260906210000 (deliver_to_customer),
-- 20260823000000 (private.prepare_delivery_signature_intent) e
-- 20260829220000 (update_service_progress).
-- Conjunto deliverable nos tres RPCs: in_service, ready, invoiced, partial_delivery.
-- Preserva duplicate_delivery_item, item_already_delivered, collection_item_not_found
-- e o ramo workshop_check_in. Nao redeclarar o wrapper publico do prepare.
...
begin
  ...
  -- workshop_check_in exige status 'collected'; delivery_term exige
  -- 'in_service', 'ready', 'invoiced' ou 'partial_delivery'
  ...
  if p_kind = 'delivery_term' and collection_record.status not in ('in_service', 'ready', 'invoiced', 'partial_delivery') then
    raise exception using errcode = 'P0001', message = 'collection_not_invoiced';
  end if;
  if collection_record.row_version <> p_expected_version then
    raise exception using errcode = '40001', message = 'stale_version';
