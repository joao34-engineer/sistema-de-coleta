-- Fase 5: 5.5 validacao de itens na entrega ao cliente.
-- Aditiva: CREATE OR REPLACE na mesma assinatura de 9 argumentos (preserva grants Chat 3).
-- Sem DROP FUNCTION. Sem unique em delivery_items (pode falhar se ja existirem duplicatas).
-- Corpo parte da PR 10 (20260906160000_phase_5_service_order_delivered_status.sql):
-- entrega parcial permanece ready; entrega total (remaining = 0) -> delivered.
-- delivery_term_items unique continua como backstop de (organization_id, collection_id, collection_item_id).
