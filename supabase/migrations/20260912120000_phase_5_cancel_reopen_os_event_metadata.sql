-- L10: metadata da transição da OS em cancel/reopen (nit §7 da auditoria).
-- A coluna service_orders.previous_status_before_cancellation continua a
-- fonte da verdade do reopen. collection_events.metadata passa a espelhar
-- o fato para diagnóstico (mesmo contrato camelCase de L4 na entrega).
-- CREATE OR REPLACE nas mesmas assinaturas. Sem DROP FUNCTION.
-- GRANT só nas duas RPCs públicas. Não toca deliver_to_customer.
-- Corpos partem de 20260907240000.
...
begin
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'row_version', next_version,
      'serviceOrderPreviousStatus', case
        when service_order_record.id is not null
        then service_order_record.previous_status_before_cancellation
        else null
      end,
      'serviceOrderStatus', case
        when service_order_record.id is not null then restored_os_status
        else null
      end
    )
  );
...
begin
  ...
  if p_action = 'cancel' then
    ...
    insert into public.collection_events (
      ...
      event_type, previous_status, new_status, reason, metadata
    ) values (
      collection_record.organization_id, p_collection_id, actor_id,
      ...
      jsonb_build_object(
        'serviceOrderPreviousStatus', case
          when service_order_record.id is not null
            and service_order_record.status is distinct from 'draft'
            and service_order_record.status is distinct from 'canceled'
          then service_order_record.status
          else null
        end
      )
    );
