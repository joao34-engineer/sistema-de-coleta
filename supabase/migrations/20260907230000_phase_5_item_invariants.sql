-- L4: invariantes de item (A5+A6).
-- remaining = vivos, não entregues, COM linha de OS. Sem OS não bloqueia delivered.
-- Unique parcial em service_order_items. item_not_ready = todas as linhas pronto.
-- create_technical_budget recusa item_id duplicado. Sem DROP FUNCTION. Sem GRANT.
-- Corpos partem de:
--   create_technical_budget ← 20260829230000
--   deliver_to_customer     ← 20260907030000
--   update_service_progress  ← 20260907030000
-- Helpers private: PR 5 herda o bloco remaining (NOT EXISTS missing check-in).
...
begin
  if exists (
    ...
    raise exception using errcode = 'P0001', message = 'duplicate_service_order_items_exist';
  end if;
end $$;

create unique index if not exists service_order_items_collection_item_uidx
  on public.service_order_items (organization_id, collection_id, collection_item_id)
  where collection_item_id is not null;
...
begin
  ...
  any_remaining_in_repair := private.any_remaining_in_repair(
    collection_record.organization_id, p_collection_id
  );
