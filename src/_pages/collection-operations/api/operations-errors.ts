...
function mapOperationsApiError(error: unknown): Omit<OperationsApiError, "actorId"> {
  ...
  if (isSupabaseError(error) && typeof error.code === "string") {
    ...
    if (error.code === "P0001" && error.message === "collection_not_invoiced") return { status: 409, code: "delivery_not_invoiced", message: "A coleta precisa estar pronta, faturada ou em entrega parcial para entregar ao cliente." };
